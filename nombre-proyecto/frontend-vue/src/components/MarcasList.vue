<template>
  <!-- c:\Users\dell\nombre-proyecto/frontend-vue/src/components/MarcasList.vue -->
  <div class="marcas-container">
    ...
    <div v-if="marcas.length > 0" class="marcas-list">
      <div v-for="marca in marcas" :key="marca.id" class="marca-item">
        <h2>{{ marca.nombre }}</h2> <!-- Cambiar 'name' por 'nombre' -->
  ...
  setup() {
    ...
    onMounted(async () => {
      try {
        ...
        error.value = `Hubo un problema al obtener las marcas: ${err.message}`;  // Mostrar el error aquí
        console.error(err);
      } finally {
        loading.value = false;
      }
    ...
    return { marcas, loading, error };  // Asegúrate de devolver `error`
  },
};
</script>
